...
const PatchParams = z
  .object({
    // Primary parameter name used by opencode.
    patchText: z.string().optional().describe("The full patch text that describes all changes to be made"),

    // Back-compat: VS Code coding agents commonly send patch text under `input`.
    input: z.string().optional().describe("Alias of patchText"),

    // Back-compat: ignored metadata.
    explanation: z.string().optional(),
  })
  .refine((v) => typeof v.patchText === "string" || typeof v.input === "string", {
    message: "patchText (or input) is required",
  })
...
export const ApplyPatchTool = Tool.define("apply_patch", {
  ...
  async execute(params, ctx) {
    const patchText = typeof params.patchText === "string" ? params.patchText : typeof params.input === "string" ? params.input : ""
    if (!patchText) {
      throw new Error("patchText (or input) is required")
    }

    // Parse the patch to get hunks
    let hunks: Patch.Hunk[]
    try {
      const parseResult = Patch.parsePatch(patchText)
